...
import { useQuote, useUpdateQuote } from "../hooks/api/useQuotes";
...
const QuoteEdit: React.FC = () => {
  ...
  // Fetch quote data
  const { data: quoteData, isLoading: isLoadingQuote } = useQuote(id || "", companyId!);
  ...
  useEffect(() => {
    if (quoteData && !isInitialized) {
      const quote = quoteData;

      // Set customer info
      setCustomerName(quote.customerName || "");
      setCustomerPhone(quote.customerContact || "");
      ...
      // Set tax
      if (quote.taxSettingId) {
        setSelectedTaxId(quote.taxSettingId);
      }
      ...
      } else if (quote.totalDiscount && quote.totalDiscount > 0) {
        setDiscountType("amount");
        setDiscountAmount(quote.totalDiscount);
      }
      ...
      if (quote.items && quote.items.length > 0) {
        const quoteItems: QuoteItem[] = quote.items.map((item: any) => ({
          ...
          stockQuantity: item.product?.currentStock || item.product?.stockQuantity || 0,
        }));
        setItems(quoteItems);
      }
  ...
  }, [items, updateQuoteMutation, id, companyId, customerName, customerPhone, customerEmail, currency, selectedTaxId, discountType, calculatedDiscount, discountPercent, validUntil, notes, navigate]);
      ...
      <Layout>
        <div className="flex items-center justify-center h-[60vh]">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-gray-600">جاري تحميل بيانات العرض...</span>
          </div>
        </div>
      </Layout>
    );
  ...
  if (quoteData?.convertedToSaleId) {
    return (
      <Layout>
        <div className="flex items-center justify-center h-[60vh]">
          <div className="text-center">
            <div className="text-red-500 text-6xl mb-4">⚠️</div>
            <h2 className="text-xl font-bold text-gray-900 mb-2">لا يمكن تعديل هذا العرض</h2>
            <p className="text-gray-600 mb-4">تم تحويل هذا العرض السعري إلى فاتورة بيع</p>
            <Button onClick={() => navigate("/quotes")}>العودة للعروض</Button>
          </div>
        </div>
      </Layout>
    );
      ...
      <div className="w-full space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4 space-x-reverse">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">تعديل العرض السعري</h1>
              <p className="text-gray-500 text-sm">
                {quoteData?.quoteNumber ? `رقم العرض: ${quoteData.quoteNumber}` : "تعديل بيانات العرض"}
              </p>
            </div>
          </div>
          <div className="flex items-center space-x-2 space-x-reverse">
            ...
            >
              ...
              {updateQuoteMutation.isPending ? "جاري الحفظ..." : "حفظ التعديلات"}
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 h-[calc(100vh-180px)]">
          ...
          <div className="lg:col-span-4 flex flex-col gap-3 overflow-y-auto">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              ...
              >
                {updateQuoteMutation.isPending ? "جاري الحفظ..." : `حفظ التعديلات - ${formatCurrency(total)}`}
              </Button>
            </div>
          </div>
        </div>
